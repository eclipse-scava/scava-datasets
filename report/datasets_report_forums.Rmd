...
**Download**: [`r file_forums_posts`.gz](`r url_forums_posts`.gz)

```{r forums_posts}
data <- read.csv(file=url_forums_posts, header=T)
```

File is [``r url_forums_posts``](../../`r url_forums_posts`), and has ``r ncol(data)`` columns for ``r nrow(data)`` posts. The evolution of posts
...
**Download**: [`r file_forums_threads`.gz](`r url_forums_threads`.gz)

```{r forums_threads}
data <- read.csv(file=url_forums_threads, header=T)
```

File is [``r url_forums_threads``](../../`r url_forums_threads`), and has ``r ncol(data)`` columns for ``r nrow(data)`` threads. A wordcloud with the main words used in threads is presented below.
